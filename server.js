...
// =============================================================
db.Fighter.remove(function(err, p) {
  if (err) {
    throw err;
  } else {
...
db.Fights.remove(function(err, p) {
  if (err) {
    throw err;
  } else {
    console.log("No Of Documents deleted:" + p);
...
db.Event.remove(function(err, p) {
  if (err) {
    throw err;
  } else {
    console.log("No Of Documents deleted:" + p);
...
function Fighters() {
  axios
    .get("http://ufc-data-api.ufc.com/api/v3/us/fighters/")
    .then(function(id) {
      ...
      fighterId.forEach(function(allFighters) {
        axios
          .get(
            "http://ufc-data-api.ufc.com/api/v3/us/fighters/" +
              allFighters +
              ".json"
          )
          .then(function(fighters) {
            console.log(
              fighters.data.first_name + " " + fighters.data.last_name
            );
...
function Events() {
  axios
    .get("http://ufc-data-api.ufc.com/api/v3/us/events/")
...
function Fights() {
  axios.get("http://ufc-data-api.ufc.com/api/v3/us/events/").then(function(id) {
    ...
    eventId.forEach(function(events) {
      axios
        .get(
          "http://ufc-data-api.ufc.com/api/v3/us/events/" + events + "/fights/"
        )
        .then(function(eventFights) {
          eventFights.data.forEach(function(fightsList) {
            db.Fights.create({
              ...
              fighter1_submissionsaverage:
                fightsList.fighter1_submissionsaverage,
              ...
              fighter2_submissionsaverage:
                fightsList.fighter2_submissionsaverage,
